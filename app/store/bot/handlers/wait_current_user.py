...
class WaitCurrentUserHandler(BaseCommandHandler):
    async def handle(self):
        ...
        player_score_active = (
            await self.app.store.game_scores.get_player_status(
                player_id=user.id, game_id=active_round.game_id
            )
        )
        if player_score_active:
            await self.app.store.game_rounds.update_round(
                round_id=active_round.id, current_player_id=upd.user_id
            )
            await self.app.store.tg_api.send_message(
                Message(
                    chat_id=self.chat_id, text=f"Отвечает {user.first_name}"
                )
            )
