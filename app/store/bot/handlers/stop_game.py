...
class StopGameHandler(BaseCommandHandler):
    async def handle(self):
        ...
        if game.is_active:
            ...
            for el in rounds:
                await self.app.store.game_rounds.update_round(round_id=el.id, is_active=False)
            ...
            await self.app.store.tg_api.edit_message_reply_markup(self.chat_id, self.update.object.message.id)
