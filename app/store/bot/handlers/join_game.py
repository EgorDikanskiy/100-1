...
class JoinGameHandler(BaseCommandHandler):
    async def handle(self):
        ...
        if not user:
            user = await self.app.store.users.create_user(tg_id=callback_query.user_id, first_name=callback_query.first_name)
        ...
        rounds = await self.app.store.game_rounds.get_game_rounds_by_game_id(game.id)
        if any(round.is_active for round in rounds):
            await self.app.store.tg_api.send_message(
                Message(
                    chat_id=self.chat_id,
